...
fn main() {
    ...
    let output_path = coverage_dir.join("_ada_coverage.dat");
...
fn resolve_gcov() -> Option<String> {
    let ada_gcov_path = env::var("ADA_GCOV_PATH")
        .ok()
        .filter(|s| !s.is_empty())?;
...
fn gcov_to_lcov(gz_path: &Path, output: &mut impl Write) -> std::io::Result<()> {
    ...
    for file_data in &data.files {
        writeln!(output, "SF:{}", file_data.file)?;
        for func in &file_data.functions {
            writeln!(output, "FN:{},{}", func.start_line, func.demangled_name)?;
        }
        for func in &file_data.functions {
            writeln!(output, "FNDA:{},{}", func.execution_count, func.demangled_name)?;
        }
